...
inline const std::vector<MaterialPreset>& get_presets() {
    static const std::vector<MaterialPreset> presets = {
        ...
        {"Heavy Ferrofluid [new][experimental]", "Strong-coupling ferrofluid. Susceptibility ~4x normal ferro + stronger Langevin saturation, so it visibly clumps onto itself even without a magnet nearby and grows taller Rosensweig spikes under one. Darker metallic tint.",
         PresetCategory::FLUIDS, SpawnSolver::MPM, MPMMaterial::HEAVY_FERRO_FLUID, 4600,0.33f,0,300,{1,0}, 0,0, {0.32f,0.28f,0.38f,1},
         0.24f, "Try small puddles near a permanent magnet. Even without M held, neighboring particles should pull toward each other harder than regular ferrofluid.", PresetQuickTab::FIELD_MEMORY, 0.00f, 0.82f, 0.68f, 0.04f},
        {"Diamagnetic Fluid [new][experimental]", "Anti-ferro liquid — negative susceptibility. Repelled from strong-field regions. Pools AT THE EDGE of a magnet, hangs back from the cursor when M is held. Pale blue.",
         PresetCategory::FLUIDS, SpawnSolver::MPM, MPMMaterial::DIAMAGNETIC_FLUID, 1800,0.33f,0,300,{1,0}, 0,0, {0.58f,0.82f,0.95f,1},
         0.26f, "Drop a blob near a scene magnet or hold M. The liquid should avoid the high-field region — watch the hollow form.", PresetQuickTab::FIELD_MEMORY, 0.04f, 0.55f, 0.78f, 0.08f},
        {"Paramagnetic Mist [new][experimental]", "Very light aerosol. Low density, weak magnetization, drifts gently along field lines. Forms wispy trails rather than clumps. Good for atmospheric magnet effects.",
         PresetCategory::FLUIDS, SpawnSolver::MPM, MPMMaterial::PARA_MIST, 400,0.36f,0,300,{1,0}, 0,0, {0.82f,0.86f,0.96f,1},
         0.22f, "Spray a cloud of it, then hold M or enable ambient — watch it align with the field direction and stream rather than pool.", PresetQuickTab::FIELD_MEMORY, 0.02f, 0.35f, 0.90f, 0.03f},
        {"Sticky Ferrofluid [new][experimental]", "High-cohesion ferro. Very damped velocity, high fluid stress. Instead of spikes it forms tight dark globules that stick to surfaces under magnetic pull. Deep matte look.",
         PresetCategory::FLUIDS, SpawnSolver::MPM, MPMMaterial::STICKY_FERRO, 5800,0.32f,0,300,{1,0}, 0,0, {0.18f,0.16f,0.22f,1},
         0.22f, "Puddle + M held = compact clump moves as one body. Try it on inclined surfaces — it sticks rather than sliding.", PresetQuickTab::FIELD_MEMORY, 0.00f, 0.68f, 0.58f, 0.05f},
        {"Superconductor [new][experimental]", "Meissner effect: below T_c (~180 K) it's a PERFECT diamagnet and expels magnetic field. Above T_c it becomes a normal metal. Cold ice-blue appearance.",
         PresetCategory::STRUCTURAL, SpawnSolver::MPM, MPMMaterial::SUPERCONDUCTOR, 8900,0.22f,0,150,{1,0}, 0,0, {0.72f,0.90f,1.00f,1},
         0.24f, "Place a cold block near a scene magnet — field visibly hollows around it. Heat it with G to destroy the Meissner state. Levitation when enough field pressure underneath.", PresetQuickTab::FIELD_MEMORY, 0.00f, 0.45f, 0.95f, 0.02f},
        {"Curie Ferromagnet [new][experimental]", "Iron-like hard magnet with sharp Curie transition (~680 K). Cold = strong ferromagnet (tall spikes, heavy pull). Heat past Curie and it becomes paramagnetic — field response essentially zero. Iron-grey.",
         PresetCategory::STRUCTURAL, SpawnSolver::MPM, MPMMaterial::CURIE_FERRO, 7800,0.26f,0,300,{1,0}, 0,0, {0.48f,0.42f,0.48f,1},
         0.26f, "Place a bar, magnetize with M, watch it pull scene ferro. Then hold G over it — the magnetism dies visibly when it crosses Curie. Cool it back and it recovers.", PresetQuickTab::FIELD_MEMORY, 0.02f, 0.72f, 0.84f, 0.06f},
        {"Eddy Copper [new][experimental]", "Non-magnetic conductor (copper analog). Heats up when it MOVES through a magnetic field — eddy currents. Warm tint. Cools quickly when still.",
         PresetCategory::STRUCTURAL, SpawnSolver::MPM, MPMMaterial::EDDY_COPPER, 8900,0.30f,0,300,{1,0}, 0,0, {0.78f,0.46f,0.22f,1},
         0.26f, "Drop a copper bar into a scene with a bar magnet, then drag it through the field region with the selection tool. Should redden from eddy heating. Hold it still and it cools off.", PresetQuickTab::FIELD_MEMORY, 0.02f, 0.95f, 0.82f, 0.04f},
        {"Hard Magnet [new][experimental]", "Permanent-magnet material with persistent magnetization (Preisach-style hysteresis). Expose to a strong field for a second or two, then release — it RETAINS its M for seconds and keeps attracting scene ferrofluid on its own. Dark magnetite look.",
         PresetCategory::STRUCTURAL, SpawnSolver::MPM, MPMMaterial::HARD_MAGNET, 7400,0.28f,0,300,{1,0}, 0,0, {0.24f,0.22f,0.34f,1},
         0.26f, "Drop a bar, hold M near it for ~2s, then release M and move away. Scene ferro should still be pulled toward the bar — you just made a permanent magnet. Lasts ~3s before relaxing. Retune alpha to taste.", PresetQuickTab::FIELD_MEMORY, 0.02f, 0.80f, 0.80f, 0.04f},
        {"Granular Sand [new][experimental]", "True sand via Drucker-Prager plasticity (Klar 2016). Return-mapping on SVD in log-strain with 35° friction cone. Forms real angle-of-repose piles, avalanches, slumps on impact. Warm sand tint.",
         PresetCategory::STRUCTURAL, SpawnSolver::MPM, MPMMaterial::SAND_GRANULAR, 1600,0.32f,0,300,{1,0}, 0,0, {0.88f,0.78f,0.50f,1},
         0.22f, "Pour sand into a container — it forms a cone, not a flat pool. Tilt the container; sand avalanches when the slope exceeds ~35°. Drop a heavy ball on a pile — it crumbles realistically.", PresetQuickTab::ALL, 0.02f, 0.92f, 0.78f, 0.03f},
        {"Phase Brittle [new][experimental]", "Ceramic-like material with phase-field fracture (Wolper-inspired CD-MPM). Energy-driven damage growth + quadratic stress degradation (1-d)^2. Cracks propagate cleanly along high-stress paths instead of smearing. Pale ceramic tint.",
         PresetCategory::STRUCTURAL, SpawnSolver::MPM, MPMMaterial::PHASE_BRITTLE, 2800,0.24f,0,300,{1,0}, 0,0, {0.82f,0.86f,0.92f,1},
         0.28f, "Drop a heavy ball onto a tile of this — crack tips should propagate cleanly rather than smearing the whole tile into dust. Vary E/fracture_rate in the mat panel for different brittleness.", PresetQuickTab::ALL, 0.04f, 0.90f, 0.70f, 0.05f},
        {"Positive Ion Cloud [new][experimental]", "Electrostatic plasma: particles with charge +1. Accelerate along the ambient E-field and repel each other / attract negative ions via the solver's own E. Warm orange tint.",
         PresetCategory::FLUIDS, SpawnSolver::MPM, MPMMaterial::POSITIVE_ION, 600,0.34f,0,300,{1,0}, 0,0, {0.95f,0.55f,0.30f,1},
         0.24f, "Enable Ambient E-field (Environment > Overlays / Debug). Drop a cloud — it streams along E. Place a negative cloud nearby; they collide.", PresetQuickTab::FIELD_MEMORY, 0.04f, 0.80f, 0.88f, 0.03f},
        {"Negative Ion Cloud [new][experimental]", "Electrostatic plasma: particles with charge -1. Mirror of positive ion — streams opposite E, attracts positives. Cool blue tint.",
         PresetCategory::FLUIDS, SpawnSolver::MPM, MPMMaterial::NEGATIVE_ION, 600,0.34f,0,300,{1,0}, 0,0, {0.40f,0.60f,0.95f,1},
         0.24f, "Pair it with a positive ion cloud and an ambient E-field — see them separate and meet at the field's null surface.", PresetQuickTab::FIELD_MEMORY, 0.04f, 0.78f, 0.90f, 0.03f},
        {"Triboelectric Rubber [new][experimental]", "Neutral rubber-like solid (q=0 initial). Placeholder for future contact-friction charging. For now acts as baseline conductor for testing the electrostatic pipeline.",
         PresetCategory::STRUCTURAL, SpawnSolver::MPM, MPMMaterial::TRIBOELECTRIC, 1100,0.34f,0,300,{1,0}, 0,0, {0.62f,0.52f,0.88f,1},
         0.26f, "Place with ion clouds to see how a rubbery surface interrupts the field. Future revision will add tribocharging on contact.", PresetQuickTab::FIELD_MEMORY, 0.02f, 0.82f, 0.82f, 0.04f},
